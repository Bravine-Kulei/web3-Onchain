...
import {
  LifecycleStepper,
  LifecycleStage } from
'../../components/common/LifecycleStepper';
import { OnChainReference } from '../../components/common/OnChainReference';
export function RequestDetail() {
  // In a real app, we'd use the ID from params. For demo, we'll just use the first request if not found.
  const { id } = useParams();
  const request = REQUESTS.find((r) => r.id === id) || REQUESTS[0];
  // Map RequestStatus to LifecycleStage
  const getStage = (status: string): LifecycleStage => {
    if (status === 'Pending') return 'Requested';
    if (status === 'Revoked' || status === 'Tampered') return 'Anchored'; // Fallback for error states
      ...
      <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-green-800 text-sm mt-1">
              ...
              {request.destUni.name}. No further action is required.
            </p>
          </div>
        </div>
      }
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="font-medium text-slate-900 flex items-center gap-2">
                  ...
                  {request.issueDate ?
                new Date(request.issueDate).toLocaleString() :
                '-'}
                </div>
              </div>
            </div>
